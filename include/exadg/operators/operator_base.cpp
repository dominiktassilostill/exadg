  ...
  Range const &                           range) const
{
  ...
  {
    const dealii::internal::MatrixFreeFunctions::DoFInfo &dof_info =
      matrix_free.get_dof_info(this->data.dof_index);
        ...
        for (unsigned int batch = 0; batch < my_batch_size; ++batch)
          {
            dealii::internal::VectorReader<Number, dealii::VectorizedArray<Number>> reader;
          /*   std::bool_constant<
              dealii::internal::is_vectorizable<VectorType, Number>::value>
              vector_selector; */
              {
                const unsigned int my_n_lanes =
                  matrix_free.n_active_entries_per_cell_batch(cell + batch);
                for (unsigned int v = 0; v < my_n_lanes; ++v)
                  {
                    const std::pair<unsigned int, unsigned int> index_start =
                      dof_info.row_starts[(cell + batch) * n_lanes + v];
                    unsigned int       index_indicators = index_start.second;
                    const unsigned int next_index_indicators =
                      dof_info.row_starts[(cell + batch) * n_lanes + v + 1]
                        .second;
                    const unsigned int *dof_indices =
                      dof_info.dof_indices.data() + index_start.first;
                    ...
                    for (; index_indicators != next_index_indicators;
                         ++index_indicators)
                      {
                        ...
                        for (unsigned int j = 0; j < indicator.first; ++j)
                          reader.process_dof(dof_indices[j],
                                             src,
                                             values_dofs[batch * dofs_per_cell +
                                                         ind_local + j][v]);
                        ...
                        // combination of the global value according to
                        // constraints
                        Number value;
                        reader.pre_constraints(
                          values_dofs[batch * dofs_per_cell + ind_local][v],
                          value);
                        ...
                        for (; data_val != end_pool; ++data_val, ++dof_indices)
                          reader.process_constraint(*dof_indices,
                                                    *data_val,
                                                    src,
                                                    value);

                        reader.post_constraints(
                          value,
                          values_dofs[batch * dofs_per_cell + ind_local][v]);
                        ++ind_local;
                      }
                ...
                for (unsigned int v = 0; v < my_n_lanes; ++v)
                  {
                    ...
                    for (; index_indicators != next_index_indicators;
                         ++index_indicators)
                      {
                        ...
                        // run through values up to next constraint
                        for (unsigned int j = 0; j < indicator.first; ++j)
                          writer.process_dof(dof_indices[j],
                                             dst,
                                             values_dofs[batch * dofs_per_cell +
                                                         ind_local + j][v]);
                        ...
                        const Number *end_pool =
                          matrix_free.constraint_pool_end(indicator.second);
                        for (; data_val != end_pool; ++data_val, ++dof_indices)
                          writer.process_constraint(*dof_indices,
                                                    *data_val,
                                                    dst,
                                                    value);

                        writer.post_constraints(
                          value,
                          values_dofs[batch * dofs_per_cell + ind_local][v]);
                        ++ind_local;
                      }
